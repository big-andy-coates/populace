 ...
 * </ul>
 * <p/>
 * Values can be retrieved via {@link org.datalorax.populace.populator.typed.TypedCollection#get(java.lang.reflect.Type)}}
 *
 * @author datalorax - 28/02/2015.
 */
public class TypedCollection<V> {
    private final Map<Type, V> specificValues;
    private final Map<Class<?>, V> subTypeValues;
    ...
    public interface Builder<T> {
        Builder withSpecificTypes(final Map<Type, ? extends T> handlers);
    ...
    public boolean equals(final Object o) {
        ...
        return defaultArrayValue.equals(config.defaultArrayValue) &&
                ...
                subTypeValues.equals(config.subTypeValues);
    }

    @Override
    public int hashCode() {
        int result = specificValues.hashCode();
        result = 31 * result + subTypeValues.hashCode();
    ...
    TypedCollection(final Map<Type, V> specificValues, final Map<Class<?>, V> subTypeValues,
                    final V defaultValue, final V defaultArrayValue) {
        Validate.notNull(subTypeValues, "subTypeValues null");
        Validate.notNull(specificValues, "specificValues null");
        Validate.notNull(defaultValue, "defaultValue null");
        Validate.notNull(defaultArrayValue, "defaultArrayValue null");
        this.subTypeValues = Collections.unmodifiableMap(new HashMap<Class<?>, V>(subTypeValues));
    ...
    private V getBaseValue(final Type type) {
        ...
        for (Map.Entry<Class<?>, V> entry : subTypeValues.entrySet()) {
